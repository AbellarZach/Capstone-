...
function getVisual(type?: string) {
  ...
  if (value.includes("resolve")) {
    return { wrap: "bg-emerald-50 text-[#16A34A]", icon: "check_circle", title: "Complaint Resolved" };
  }
  if (value.includes("hearing") || value.includes("schedule")) {
    return { wrap: "bg-amber-50 text-[#D97706]", icon: "calendar_month", title: "Hearing Scheduled" };
  }
  if (value.includes("approv")) {
    return { wrap: "bg-emerald-50 text-[#059669]", icon: "description", title: "Complaint Approved" };
  }
  if (value.includes("review") || value.includes("status")) {
    return { wrap: "bg-emerald-50 text-[#059669]", icon: "mail", title: "Complaint Reviewed" };
  }
  if (value.includes("submit")) {
    return { wrap: "bg-emerald-50 text-[#16A34A]", icon: "check_circle", title: "Complaint Submitted" };
  }
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-start gap-4 py-4 text-left transition ${
                    unread ? "bg-emerald-50/40" : "hover:bg-slate-50"
                  }`}
                >
                  <span
